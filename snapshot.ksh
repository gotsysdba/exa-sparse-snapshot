...
# Create a tmdir to store files
typeset -r SPARSE_TMP=$(mktemp -d)
print -- "Storing files ${SPARSE_TMP}"
#------------------------------------------------------------------------------
# MAIN
#------------------------------------------------------------------------------
...
print -- "Running: ${CRS_HOME}/bin/crsctl status resource -f -w \"((TYPE = ora.database.type) AND (USR_ORA_DB_NAME = ${MYTESTMASTER}) AND (LAST_SERVER = ${HOST}))\""
${CRS_HOME}/bin/crsctl status resource -f -w "((TYPE = ora.database.type) AND (USR_ORA_DB_NAME = ${MYTESTMASTER}) AND (LAST_SERVER = ${HOST}))" |
while IFS="=" read KEY VALUE; do
	case ${KEY} in
		...
		USR_ORA_INST_NAME)	typeset -r INSTANCE_NAME=${VALUE} ;;
	esac
done
if [[ -z ${DB_UNIQUE_NAME} ]] || [[ -z ${ORACLE_HOME} ]]; then
...
print -- "asmcmd --nocp --privilege sysdba ls -lg +${MYDSTDG}/${DB_UNIQUE_NAME}/*" >> ${MKDIR_FILE}

# Generate DB Rename Script
typeset -r RENAME_FILE="${SPARSE_TMP}/df_rename.sql"
typeset -r RENAME_SQL="select 'EXECUTE dbms_dnfs.clonedb_renamefile ('||''''||name||''''||
						','||''''||replace(replace(name,'.','_'),'${MYSRCDG}/','${MYDSTDG}/')||'_${NOW}'''||
						');' from v\$datafile;"
...
print -- "####################################################################"
print -- "Creating ASM Directories with ${SPARSE_TMP}/asm_mkdir.sh"
sh ${MKDIR_FILE}
...
run_sql "alter system set db_create_file_dest='+${MYDSTDG}';"
run_sql "@${SPARSE_TMP}/df_rename.sql"
RC=$?
